...
{
    return (Sinv[state_byte ^ key_byte]);
}
...
int main()
{
        ...
        */
        ...
        for (i = 0; i < AES_BLOCK_SIZE; i++)
        {
            for (uint16_t key_byte = 0; key_byte < 256; key_byte++)
            {
                ...
                for (j = 0; j < 256; j++)
                {
                    reversed_byte = partial_dec(lambda_set[j][i], key_byte);
                    sum ^= reversed_byte;
                }
                /*case not candidate*/
        ...
        falsePov = detect_false_positive(possible_keys);
        if (falsePov)
        {
    ...
    prev_aes128_round_key(guessed_key, tmp_key, 3);
    prev_aes128_round_key(tmp_key, guessed_key, 2);
    prev_aes128_round_key(guessed_key, tmp_key, 1);
    prev_aes128_round_key(tmp_key, guessed_key, 0);
    printf("key : \n");
