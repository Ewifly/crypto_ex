...
int test_1_alternative()
{
    ...
    next_aes128_round_key_alternative(test_key, next_key, round);
    prev_aes128_round_key_alternative(next_key, prev_key, round);
...
int test_2_alternative()
{
    ...
    for (int i = 0; i < 256; i++)
    {
        keyFunction_alternative(lambda[i], key1, key2);
    }
...
int main()
{
    int t1 = test_1_alternative();
    int t2 = test_2_alternative();
    return 0;
}