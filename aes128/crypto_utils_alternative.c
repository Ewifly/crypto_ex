...
int keyFunction_alternative(uint8_t block[AES_BLOCK_SIZE], const uint8_t key1[AES_128_KEY_SIZE], const uint8_t key2[AES_128_KEY_SIZE])
{
	/*Copy Input Block */
	...
	aes128_enc_alternative(block, key1, 3, 1);
	aes128_enc_alternative(block2, key2, 3, 1);
	/*Xor the two resultant block to get the final result*/
	xor_block(block, block2);
	return 0;
}

int generate_key_alternative(uint8_t key[AES_128_KEY_SIZE])
{
